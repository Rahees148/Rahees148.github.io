...
let IssuesService = class IssuesService {
    ...
    getOneIssue(id, fieldsString) {
        return this.http
            ...
            return new src_app_shared_models_issue_model__WEBPACK_IMPORTED_MODULE_5__["Issue"](this.clearCustomFields(r));
        }));
    }
    getResolversList() {
        return this.http.get(this._constant.getResolverList).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["map"])(r => {
            return r;
        }));
    }
    raiseAnIssue(data) {
    ...
    clearCustomFields(resp) {
        let customFieldName = "customfield";
        let nObject = {};
        // not necessary var
        let numberOfCustomFields = 0;
...
let DialogRaisanissueComponent = class DialogRaisanissueComponent {
...
DialogRaisanissueComponent.ctorParameters = () => [
...
let IssueDetailsComponent = class IssueDetailsComponent {
    constructor(fb, dialog, _restApi, activatedRoute) {
        this.fb = fb;
        this.dialog = dialog;
    ...
    getDetails() {
        let fieldsString = `?fields=${Object.keys(new _shared_models_issue_model__WEBPACK_IMPORTED_MODULE_6__["IssueFields"]()).join(",")},${Object.keys(_shared_models_custom_issue_fields_enum__WEBPACK_IMPORTED_MODULE_10__["CustomFields"])
            .map(k => _shared_models_custom_issue_fields_enum__WEBPACK_IMPORTED_MODULE_10__["CustomFields"][k])
            .join(",")}`;
        Object.keys(new _shared_models_issue_model__WEBPACK_IMPORTED_MODULE_6__["IssueFields"]()).forEach(r => console.log(r));
        ...
        this._restApi
            ...
            console.log(this.editForm.get("status"));
            this.StatusCtrl.setValue(this.issue.Status);
        }, err => this._restApi.errorHandler(err));
    }
    getResolversList() {
        this._restApi.getResolversList().subscribe((r) => {
            // TODO response interface
            console.log("resoveler list: ", r);
            this.resolversList = r.actors;
        }, err => this._restApi.errorHandler(err));
    }
    ...
    submitForm() {
        ...
        let downstreamActs = this.editForm.get("downStreamActs").value;
        console.log("values d.acts: ", downstreamActs);
        const dialogRef = this.dialog.open(_shared_components_dialog_dialog_component__WEBPACK_IMPORTED_MODULE_3__["DialogOverviewExampleDialog"], {
            width: "870px",
    ...
    createDownStreamActivities() {
        let arr = [];
        console.log(this.downstreamActivities);
        Object.keys(this.downstreamActivities).forEach(act => {
            arr.push(this.fb.control(false));
        });
        console.log(arr);
        return this.fb.array(arr);
    }
    createRootCauseCategories() {
        let arr = [];
        Object.keys(this.rootCauseCategory).forEach(act => {
            arr.push(this.fb.control(false));
        });
        console.log(arr);
        return this.fb.array(arr);
    ...
    getActivityTitle(index) {
        return this.downstreamActivities[Object.keys(this.downstreamActivities).filter((v, i) => {
            if (i === index)
                return v;
        })[0]];
    }
    getCategoryTitle(index) {
        return this.rootCauseCategory[Object.keys(this.rootCauseCategory).filter((v, i) => {
            if (i === index)
                return v;
        })[0]];
    }
};
IssueDetailsComponent.ctorParameters = () => [
...
let RaiseAnIssueComponent = class RaiseAnIssueComponent {
    constructor(formBuilder, dialog, _constant, _restAPI, router) {
    ...
    ngOnInit() {
        const descriptioanPlaceHolder = `1. Instrument (COA) name:
...
4. eCOA Administration: site/ home/ site and home`;
        ...
        this.filteredStudyNumberOptions = this.raiseAnIssueForm.get("studyNumber").valueChanges.pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_7__["startWith"])(''), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_7__["map"])(value => this._filterOptions(value, this.studyNumberoptions)));
        this.filteredVendorOptions = this.raiseAnIssueForm.get("vendor").valueChanges.pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_7__["startWith"])(''), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_7__["map"])(value => this._filterOptions(value, this.vendorOptions)));
        this.addImpactsCheckboxes();
    }
    // convenience getter for easy access to form fields
    get f() { return this.raiseAnIssueForm.controls; }
    onSubmit() {
        this.submitted = true;
        ...
        const selectedIssueImpact = this._getSelectedIssueImpact();
        let data = {
    ...
    openDialog() {
        const dialogRef = this.dialog.open(_dialog_raisanissue_dialog_raisanissue_component__WEBPACK_IMPORTED_MODULE_8__["DialogRaisanissueComponent"], {
            width: '870px',
    ...
    _filterOptions(value, Listoptions) {
        const filterValue = value.toLowerCase();
        return Listoptions.filter(option => option.toLowerCase().includes(filterValue));
    }
    _getSelectedIssueImpact() {
        return this.raiseAnIssueForm.value.impacts
            .map((v, i) => v ? this.IssueImpacts[i].value : null)
            .filter((v) => v !== null);
    }
    addImpactsCheckboxes() {
        this.IssueImpacts.forEach((_o, i) => {
            const control = new _angular_forms__WEBPACK_IMPORTED_MODULE_3__["FormControl"]();
            this.raiseAnIssueForm.controls.impacts.push(control);
        });
    }
};
RaiseAnIssueComponent.ctorParameters = () => [
...
const ifDefinedCustomField = (fieldName) => {
    return Object.keys(CustomFields).some((k) => CustomFields[k] === fieldName);
};
...
class Issue {
    ...
    deserialize(issue) {
        ...
        if (issue.fields.comment &&
            ...
            this.fields.comment.comments = issue.fields.comment.comments.map((comment) => new Comment(comment));
...
let TableListComponent = class TableListComponent {
    constructor(_restAPI, _constant) {
        this._restAPI = _restAPI;
        this._constant = _constant;
    ...
    getAllassignedData() {
        this._restAPI.getAllAssignedTicket().subscribe(data => {
            this.tableData = data;
            this.dataSource = new _angular_material_table__WEBPACK_IMPORTED_MODULE_4__["MatTableDataSource"](this.tableData.issues);
    ...
    getAllTicketReportedByme() {
        this._restAPI.getAllTicketReportedByme().subscribe(data => {
            this.tableData = data;
            this.dataSource = new _angular_material_table__WEBPACK_IMPORTED_MODULE_4__["MatTableDataSource"](this.tableData.issues);
            this.dataSource.paginator = this.paginator;
            this.dataSource.sort = this.sort;
